...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=16, fig.height=7, fig.align='center', fig.cap="Fig. 2.6: Importance of different people involved in the decision-making process"}
...
data %>%
  ...
  dplyr::select(-sum) %>%
  mutate(start = -prop_cum[importance == 2],
         end_adj = start + prop_cum, 
         start_adj = end_adj - prop,
         answer = as.factor(importance),
         item = as.numeric(parse_number(party))) %>%
  ggplot() +
  geom_rect(aes(x = party,
                ymin = start_adj,
                ymax = end_adj,
                xmin = item + 0.4,
                xmax = item - 0.4,
  ...
  geom_text(aes(x = party, y = start_adj + 2, label = paste0(prop, "%")), size = 3.5, face = "bold") +
  labs(title = " ", 
       y = "Proportion endorsing a response (%)",
       x = " ",
       fill = "Response",
       caption = c("Least important", "Most important")) +
