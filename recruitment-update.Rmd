...
* Number of participants who consented to participate: `r sum(raw_data$consent, na.rm = TRUE)`.

* Number of participants who completed the survey: `r raw_data %>% filter(lastpage %in% c(9:10)) %>% nrow()` or `r round((raw_data %>% filter(lastpage %in% c(9:10)) %>% nrow())/sum(raw_data$consent, na.rm = TRUE)*100,1)`% of those who consented.
